...
interface POSFormData {
  ...
  terminalId: string;
  apiKey: string;
  settlementTime: string; // Gün sonu saati
  status: 'active' | 'inactive';
  installmentOptions: number[]; // Taksit seçenekleri
}

function EditPOSModal({ isOpen, onClose, pos }: Props) {
  const [formData, setFormData] = useState<POSFormData>({
    name: pos.name,
    type: pos.type as 'single' | 'installment',
    merchantId: '123456',
    terminalId: '001',
    apiKey: '********',
    settlementTime: '', // Başlangıç değeri
    status: pos.status as 'active' | 'inactive',
    installmentOptions: [], // Başlangıç değeri boş dizi
  });
